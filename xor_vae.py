...
def generate_xor_data_with_noise(size, noise_level=0.1):
    x1 = torch.randint(0, 2, (size, 1, 1), dtype=torch.float32)
    x2 = torch.randint(0, 2, (size, 1, 1), dtype=torch.float32)
    y = torch.logical_xor(x1.bool(), x2.bool()).float().view(-1)  # Target is XOR of x1 and x2
...
x1_noisy, x2_noisy, y = generate_xor_data_with_noise(100)
...
title = f'XOR PCA of VAE Latent Space with L1 Norm = {lambda_l1}'
plt.title(title)
