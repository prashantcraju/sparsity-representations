...
N = 5
M = 10
sparsity_values = [0.1 * i for i in range(1, 10)]  # Sparsity from 0.1 to 0.9
tasks = ['XOR', 'Input1', 'Input2']
beta1, beta2 = 0.9, 0.1  # Adjust these values as needed
...
for sparsity in sparsity_values:
    ...
    for task in tasks:
        ...
        plt.subplot(1, 4, 1)
        plt.plot(losses)
        plt.title(f'Loss for {task}, Sparsity {sparsity:.1f}')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ...
        plt.tight_layout()
        plt.savefig(f'{task}-Sparsity-{sparsity:.1f}.png')
        plt.show()
