...
def generate_discrimination_data(batch_size, sparsity=0.5):
    """
    Generate data for input discrimination task with a given level of sparsity.
    ...
    """
    # Randomly generate binary data for x1 and x2
    x1 = torch.randint(0, 2, (batch_size, 2), dtype=torch.float32)
    x2 = torch.randint(0, 2, (batch_size, 2), dtype=torch.float32)

    # Targets are 1 where both elements of x1 are 1
    targets = (x1[:, 0] == 1) & (x1[:, 1] == 1)
...
# sparsity = 0.3   # 30% of the data will be zeros
# x1, x2, targets = generate_discrimination_data(batch_size, sparsity)
...
for i in range(6):
    ...
    x1, x2, targets = generate_discrimination_data(batch_size, i*.2)
    ...
    for epoch in range(1000):
        ...
        if epoch % 250 == 0:
            ...
            plot_layer_outputs(model, x1, x2,  f'X1 Discrimination Task Sparsity {sparsity}')  # Visualize layer outputs
