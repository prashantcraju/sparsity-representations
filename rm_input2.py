...
# Define the multimodal model with L1 loss constraint
...
def train_model(samples, task, sparsity_level):
    ...
    for _ in range(epochs):
        ...
        representations.append(out2.detach().numpy())  # Detach and convert to numpy array
...
def multimodal_task(samples, sparsity_level):
    representations = train_model(samples, 'input2', sparsity_level)
...
# def multimodal_task(samples, sparsity_level):
#     representations = train_model(samples, 'input1', sparsity_level)
#     rdm = compute_rdm(representations)
#     return rdm
...
for i, sparsity_level in enumerate(sparsity_levels):
    ...
    axs[row, col].imshow(rdm_input1, cmap='viridis')
