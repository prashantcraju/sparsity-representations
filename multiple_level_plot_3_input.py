...
class MultimodalModel(nn.Module):
    def __init__(self, input_size1, input_size2, input_size3, hidden_size):
        ...
        self.relu = nn.ReLU()
        self.fc4 = nn.Linear(hidden_size * 3, 3)  # Output size is 3 for 3D visualization

    def forward(self, x1, x2, x3):
        out1 = self.relu(self.fc1(x1))
        out2 = self.relu(self.fc2(x2))
        out3 = self.relu(self.fc3(x3))
        combined = torch.cat((out1, out2, out3), dim=1)  # Concatenate along the feature dimension
...
def multimodal_task(input1_samples, input2_samples, input3_samples, input1_dim, input2_dim, input3_dim, sparsity_levels, epochs=1000):
    # Initialize empty list to store representations for each sparsity level
    all_representations = []

    # Define the model and optimizer
    model = MultimodalModel(input_size1=input1_dim, input_size2=input2_dim, input_size3=input3_dim, hidden_size=32)
...
def plot_all_representations(all_representations, sparsity_levels):
    ...
    ax.legend()
    ax.set_title('Multimodal Representational Geometry vs. Sparsity Level (3 inputs)')
    plt.show()
...
representations = multimodal_task(input1_samples, input2_samples, input3_samples, input1_dim, input2_dim, input3_dim, sparsity_levels, epochs)
